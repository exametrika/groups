* ReceiveQueue
 - на флаш мы игнорируем ордер совершенно, не нарушает ли это свойство durability протокола, если нет может порядок слать более легким мультикастом, сейчас для durable сообщений посылаются durable  ordered сообщения, для failure atomic - failure atomic сообщения
* расширить flushcondition должен уметь проверять входящие и упавшие/вышедшие
* totalorderprotocol 
  - на флаш формировать порядок сообщений без ордера детерминированным образом(по ид отправителя и сообщения например)
* тесты
  - IFlushCondition
