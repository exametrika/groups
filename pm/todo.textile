* нужно проверить, что посылать и принимать Failure atomic сообщения (посланные группе в целом) можно только после входа узла в группу, при этом стоит учесть, что конечный membership на узле получателе может быть еще не установлен, после того, как сообщение, посланное с узла с установленным членством будет принято. Т.е. принимать можно, складывая в pending, если членство еще не установлено, а послыать только после установки первого flush (joined event) 
* расширить flushcondition должен уметь проверять входящие и упавшие/вышедшие
-----------
...
* нужно воспроизвести в тестах flowControl - не заблокирует ли он отправку и прием системных сообщений при заблокированных прикладных  
* тесты
  - IFlushCondition
